...
fn about(context: &mut Context, msg: &Message) -> CommandResult {
  let (invite_url, face) = {
    ...
    match context.cache.read().user.invite_url(&context, Permissions::READ_MESSAGES | Permissions::SEND_MESSAGES |
        Permissions::EMBED_LINKS | Permissions::ADD_REACTIONS | Permissions::READ_MESSAGE_HISTORY |
        Permissions::USE_EXTERNAL_EMOJIS | Permissions::CONNECT | Permissions::USE_VAD |
    ...
    let face = if msg.mentions.is_empty() {
        if args.is_empty() {
            ...
            let result: Result<String, Box<dyn std::error::Error>> = try {
                ...
                .read().members_starting_with(args.rest(), false, true).first().ok_or("Could not find member")?
                .user_id().to_user(&context)?.face()
            };
            match result {
                Ok(face) => face,
                Err(e) => {
